...
def main(argv=None):
    """
    Ce script implemente le dechiffrement DES suivi du decodeur 3
    Options :
    ...
    oldpin_bin = bin(ord(code[2]))[2:].zfill(8) \
        + bin(ord(code[3]))[2:].zfill(8)
    parity_chk = str(oldpin_bin[:7].count('1') % 2)
    parity_chk += str(oldpin_bin[7:14].count('1') % 2)

    if (oldpin_bin[-2:] != parity_chk):
        print 'Erreur dans la transmission'
        return 4

    pin = int(pin_bin[:14], 2)
    oldpin = int(oldpin_bin[:14], 2)

    sys.stdout.write(str(pin) + '\t' + str(oldpin).zfill(4))
